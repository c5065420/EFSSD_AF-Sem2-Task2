...
def create_trip(user, match_title, competition, match_date, kickoff_time,
    ...
    cur = conn.cursor()
    cur.execute('''
        INSERT INTO trips (user, match_title, competition, match_date, kickoff_time,
...
def update_trip(trip_id, match_title, competition, match_date, kickoff_time,
    ...
    cur.execute('''
        UPDATE trips
        SET match_title=?, competition=?, match_date=?, kickoff_time=?,
            departure_point=?, departure_time=?, destination=?,
            total_seats=?, price=?, notes=?, poster=?
        WHERE id=?
...
def get_bookings_by_user(user_id):
    ...
    cur.execute('''
        SELECT b.id, b.seats, b.created,
               ...
               t.kickoff_time, t.departure_point, t.destination, t.price, t.user AS organiser
        FROM bookings b
